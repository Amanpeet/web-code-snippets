...
function realhero_ajax_subscribe()
{
  ...
  if (!empty($fields)) :
    ...
    if ($count > 0) {
      $output = array(
        'status'    => 'error',
        'msg'       => __('Already in a database.', undercustoms)
      );
    } elseif (!defined('NEWSLETTER_VERSION')) {
      $output = array(
        'status'    => 'error',
        'msg'       => __('Please install & activate newsletter plugin.', undercustoms)
      );
    } else {
      /**
       * Generate token
    ...
    $output = array(
      'status'    => 'error',
      'msg'       => __('An Error occurred. Please try again later.', undercustoms)
    );
  endif;
